...
const getSystemPrompt = (currentDateStr, currentYear) => `You are a strict, automated editorial assistant crafting an engaging news post for Instagram.
...
`;
export async function runAutomatedPipeline(ctx, userInput, uploadedMedia) {
    try {
            ...
            tools: {
                ...
                scrapeUrl: tool({
                    ...
                    execute: async ({ url }) => {
                        ...
                        return res.markdown || JSON.stringify(res);
                    },
                }),
            },
...
`,
        ...
        if (!coverImageUrl) {
            ...
            const { files } = await generateText({
                ...
                messages: [{ role: 'user', content: [{ type: 'text', text: imageGenerationPrompt }] }],
            });
        ...
        if (finalCaption.length > 1024) {
            ...
            previewMsg = await ctx.replyWithPhoto({ source: imageBuffer }, { caption: finalCaption });
        }
        ...
        if (hasVideo) {
            ...
            if (uploadedMedia && uploadedMedia.length > 0) {
                ...
                for (const m of uploadedMedia) {
                    if (!m.s3Url)
                        continue; // Skip if processing failed in Phase 1
