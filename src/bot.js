...
const mediaGroupAccumulator = new Map();
...
async function handleMediaMessage(ctx, isVideo) {
    const chatId = ctx.chat.id.toString();
    if (activeProcessing.has(chatId)) {
        return; // Ignore if already processing
    }
            ...
            mediaGroupAccumulator.set(mediaGroupId, {
                ...
                timer: setTimeout(async () => {
                    ...
                    try {
                        ...
                        try {
                            await ctx.reply('Terjadi kesalahan sistem saat memproses album.');
                        }
                        catch (e) { }
                    }
                    finally {
    ...
    runAutomatedPipeline(ctx, text, [{ type: isVideo ? 'video' : 'image', url: fileLink.toString(), mimeType }])
        ...
        try {
            await ctx.reply('Terjadi kesalahan sistem saat memproses media.');
        }
        catch (e) { }
    })
        .finally(() => {
